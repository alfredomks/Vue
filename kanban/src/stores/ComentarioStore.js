...
export const useComentarioStore=defineStore('ComentarioStore',{
    ...
    getters:{
        getLog(state){
            return state.lista_comentarios;
        }
    },
    actions:{
        getComentarios(){
            var pagina=this.pagina-1;
            fetch('http://localhost:8080/demo-0.0.1-SNAPSHOT/api/v1/comentarios/get?tarea='+this.tarea.id+'&page='+pagina+'&size=10')
            .then(response => response.json())
            .then(data => {
                this.numero_paginas=data.totalPages;
                this.lista_comentarios=data.content;
                console.log(data);
            })
            .catch(error => {
        ...
        addComentario(){
            ...
            const requestOptions = {
              ...
              fetch('http://localhost:8080/demo-0.0.1-SNAPSHOT/api/v1/comentarios/'+this.tarea.id, requestOptions)
                .then(async response => {
        ...
        delteComentario(item){
            const url = 'http://localhost:8080/demo-0.0.1-SNAPSHOT/api/v1/comentarios/'+item.id;
            const requestOptions = {
                method: 'DELETE',
                headers: {
